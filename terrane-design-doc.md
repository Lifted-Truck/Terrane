...
- **Destination (target, set 2026-06-13):** a **VST3/AU plugin compatible with Ableton Live**, via JUCE, once the dynamics model is proven by play. This is now the north star, not a someday: every design decision should be checked against it.
  ...
  - **The load-bearing fork — Tonality is Python; a distributable plugin is native.** A real-time AU/VST3 in Ableton cannot ship a CPython dependency or a sidecar subprocess. Since TERRANE only consumes a handful of Tonality functions, all at harmonic-rhythm rate (key induction = Krumhansl–Schmuckler correlation against the versioned `kk-1982.1` profiles; `voice_leading_realized`; `dft_magnitudes` evenness; chord identity — all small, deterministic, table-driven over the 4096 pc-sets), the intended resolution is to **embed Tonality's exact data (profile tables, set-class tables) and a faithful C++ port of just those functions in the plugin**, preserving "reduce, never invent" by reusing Tonality's data and algorithms rather than re-deriving them. To be coordinated with the Tonality project — see §11. **Tonality already plans this**: its *Decision 10 ("port once, bind back")* is a single C++ core with Python bindings (Phase 8), explicitly driven by native plugin/device consumers, with the core identity layer as `constexpr` tables over the 4096 pc-sets and a golden conformance harness already in place. So the likely native path is **consuming Tonality's shared C++ core**, not a TERRANE-maintained port. Critical dependency: the shared core is fenced behind Tonality's *Phase 6 (12-TET surface freeze)* — "porting first means porting twice" — so TERRANE's JUCE port cannot assume a native Tonality core before then.
  - **Ruling (response-3, 2026-06-13) — the "consumer-port corollary":** TERRANE *may* maintain a faithful native port of the subset it uses *in the interim*, sanctioned (not a drift-prone fork) because parity is bounded by two contracts: (1) **versioned data + documented algorithm, citing versions** — `kk-1982.1` key profiles are portable JSON (load and pin); DFT magnitudes, set-class/prime-form, and the `doubling.1` non-crossing pairing are deterministic algorithms over the 4096-mask space (reimplement from the documented algorithm). Tonality will publish a stable-schema versioned-data export (priors + a *generated* precomputed set-class/DFT table artifact), **pullable forward** ahead of the full port. (2) **Parity is mechanically checkable** against Tonality's golden conformance harness (`tests/golden/conformance.json`, rel 1e-9) — a TERRANE port is "faithful" iff it reproduces the relevant golden cases (`key_induction`, `realized_voice_leading`, `set_class_info`, naming). When Tonality's Phase 8 C++ core lands, **link it** instead — same binary brain, zero drift; the interim port is a bridge to that. **Action when the JUCE port begins:** send a brief stating start timing and whether we want the generated table artifact or just the documented algorithms (they prioritize the export then). TERRANE is recorded as the motivating native consumer.
...
> **Status (2026-06-11; native-export round 2026-06-13):** the relay below happened and Tonality answered; intake is closed — see `integrations/terrane/` in the Tonality repo (briefs + responses, numbered rounds). brief-3/response-3 cover the VST3/AU plugin target and rule the native-export path (consumer-port corollary; see §10). Verdicts: items 1 and 3 (evenness) shipped/documented; VL pairing shipped at identity level (realization level = Tonality gap 6); cadence events = gap 7; streaming = gap 5; TERRANE recorded as target application A5. Also shipped: spelled/labeled display views (`mts/context/result_format.py`) and the full 6-D DFT embedding (T_n/T_nI-invariant) for use as terrain coordinates. Build against INTEGRATION.md's documented recipes; pin key-profile version `kk-1982.1`.
